...
def split_sentences(text: str) -> list[str]:
    ...
    for para in paragraphs:
        for part in _SENTENCE_SPLIT_RE.split(para.strip()):
            part = part.strip()
            if part:
...
def chunk_sentences(sentences: list[str], *, min_words: int, max_words: int) -> list[str]:
    ...
    for sentence in sentences:
        ...
        if sentence_words > max_words:
            flush()
            chunks.extend(_split_long_sentence(sentence, max_words=max_words))
            continue
...
def _suspected_truncation(
    *, duration_seconds: float | None, word_count: int, words_per_second: float
) -> tuple[bool, str | None]:
...
class TTSSelector(BaseTool):
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        if (
            ...
            if enabled is True or (enabled is not False and estimated_seconds > threshold_seconds):
                result = self._execute_fish_speech_chunked(tool, inputs)
            else:
    ...
    def _execute_fish_speech_chunked(self, tool: BaseTool, inputs: dict[str, Any]) -> ToolResult:
        ...
        for i, chunk_text in enumerate(chunks_text, start=1):
            chunk_output_path = (chunk_dir / f"{prefix}_c{i:02d}.wav").resolve()
        ...
        if out_ext == "wav":
            ...
            cmd = [ffmpeg, "-y", "-f", "concat", "-safe", "0", "-i", str(concat_list_path), "-c:a", "libmp3lame", "-q:a", "4", str(merged_path)]
        else:
            cmd = [ffmpeg, "-y", "-f", "concat", "-safe", "0", "-i", str(concat_list_path), str(merged_path)]
